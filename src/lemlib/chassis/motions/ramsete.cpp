...
void lemlib::Chassis::ramsete(const asset& path, float beta, float zeta, int timeout, bool forwards, bool async) {
    ...
    if (pathPoints.size() == 0) {
        lemlib::infoSink()->error("No points in path! Do you have the right format? Skipping motion");
        distTraveled = -1;
        this->endMotion();
        return;
    }
    ...
    for (int i = 0; i < timeout / 10 && pros::competition::get_status() == compState && this->motionRunning; i++) {
        ...
        lemlib::Pose targetPose = pathPoints.at(closestPointIndex);
        lemlib::Pose nextPose = pathPoints.at(closestPointIndex + 1);
        ...
        lemlib::Pose interpolatedTarget = targetPose.lerp(nextPose, t);
        ...
        float headingError = lemlib::angleError(interpolatedTarget.theta, pose.theta, true);
