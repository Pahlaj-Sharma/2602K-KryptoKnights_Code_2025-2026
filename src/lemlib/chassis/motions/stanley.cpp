 ...
 */
std::pair<int, float> findClosestPoint(const lemlib::Pose& pose, const std::vector<lemlib::Pose>& path, int lastIndex) {
    ...
    for (size_t i = lastIndex; i < std::min((size_t)lastIndex + lookahead, path.size() - 1); ++i) {
        const lemlib::Pose& startPoint = path[i];
        const lemlib::Pose& endPoint = path[i + 1];
 ...
 */
PursuitError calculateErrors(const lemlib::Pose& pose, const std::vector<lemlib::Pose>& path, int i, float t,
                             float lookahead) {
    ...
    const float currentHeadingError = lemlib::angleError(pose.theta, currentHeading);
    const float futureHeadingError = lemlib::angleError(pose.theta, futureHeading);
...
void lemlib::Chassis::stanley(const asset& path, float lookahead, int timeout, bool forwards, bool async,
    ...
    std::vector<lemlib::Pose> pathPoints = getData(path);
    ...
    const lemlib::Pose& finalPoint = pathPoints.back();
