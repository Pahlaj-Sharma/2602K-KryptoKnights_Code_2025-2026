...
pahlib::TrackingWheel::TrackingWheel(pros::adi::Encoder* encoder, float wheelDiameter, float distance,
                                     float gearRatio) {
...
pahlib::TrackingWheel::TrackingWheel(pros::Rotation* encoder, float wheelDiameter, float distance, float gearRatio) {
...
pahlib::TrackingWheel::TrackingWheel(pros::MotorGroup* motors, float wheelDiameter, float distance, float rpm) {
    this->motors = motors;
...
void pahlib::TrackingWheel::reset() {
...
float pahlib::TrackingWheel::getDistanceTraveled() {
    if (this->encoder != nullptr) {
        ...
        return pahlib::avg(distances);
    } else {
        return 0;
    }
}

float pahlib::TrackingWheel::getOffset() { return this->distance; }

int pahlib::TrackingWheel::getType() {
